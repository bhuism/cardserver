...
@Slf4j
public record AiPlayerNew(List<Card> currentTrick, Suit trumpSuit, Hand hand, GameVariant gameVariant) {
...
//        final String _partnerId = gameEngine.getPartner(_userId);
//        final String _currentWinnerId = gameEngine.getTrickWinnerId(currentTrick);


        boolean isPartnerWinning;
        ...
        if (hand.hasSuit(trumpSuit)) {
            ...
            if (trickContainsTrump && !isPartnerWinning) {
                if (gameVariant == GameVariant.ROTTERDAMS) {
                    ...
                    return discardCardWithSignal(hand.bySuit());
                } else {
     ...
     */
    private Card discardCardWithSignal(final Map<Suit, List<Card>> cardsBySuit) {
        ...
        Optional<Card> lowestCardToDiscard = hand.cards.stream()
            .filter(c -> c.getSuit() != trumpSuit)
            .min(this::compareKlaverjassenCards);

        // Strategy 3: Only trumps are left or no other option. Must discard the lowest card overall.
        return lowestCardToDiscard.orElseGet(() -> hand.cards.stream()
            .min(this::compareKlaverjassenCards)
            .orElseThrow());
    }
    ...
    public Card getHighestCardInTrick() {
...
//            .getPlayers()
//            .indexOf(userId);
//        return gameEngine.getGame().getPlayerCard()
//            .entrySet()
//            .stream()
//            .filter(entry -> entry.getValue()
//                .equals(playerNum))
...
//    }
    ...
    private int getKlaverjassenRank(final Card card) {
        final boolean isTrump = card.getSuit().equals(trumpSuit);
        if (isTrump) {
    ...
    public boolean decideBid() {

        int handStrength = 0;


        // 1. Evaluate trump suit strength
        final List<Card> trumpCards = hand.bySuit.getOrDefault(trumpSuit, List.of());
        ...
        for (Map.Entry<Suit, List<Card>> entry : hand.bySuit.entrySet()) {
            if (entry.getKey() == trumpSuit) {
                continue;
            }
        ...
        for (Suit s : Suit.values()) {
            ...
            int suitSize = hand.bySuit.getOrDefault(s, List.of())
                .size();
            if (suitSize == 1) {
        ...
        log.info("evaluates their hand for trump: {}, with strength: {}, decision={}", trumpSuit, handStrength, decision);
        return decision;
    }
