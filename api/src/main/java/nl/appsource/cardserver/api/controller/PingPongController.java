...
//        log.info("{} ping() ", exchange.getRequest().getRemoteAddress());
        return getUserId(exchange)
            ...
            .flatMap(userId -> pingPongSchema.map(PingPongSchema::getAppIdentifier)
                .flatMap(appIdentifier -> sseSessionRepository.existsById(appIdentifier).flatMap(exists -> {
                        if (!exists) {
                    ...
                    }).then(Mono.defer(() -> reactiveCouchbaseTemplate.getCouchbaseClientFactory().getDefaultCollection().reactive().mutateIn(appIdentifier, Arrays.asList(MutateInSpec.increment("pingReceivedCount", 1),
                            MutateInSpec.upsert("pingReceived", currentTimeMillis())), MutateInOptions.mutateInOptions().expiry(Duration.ofSeconds(15)))
        ...
        return getUserId(exchange)
            .flatMap(userId -> pingPongSchema.map(PingPongSchema::getAppIdentifier)
                .flatMap(appIdentifier -> sseSessionRepository.existsById(appIdentifier).flatMap(exists -> {
                    ...
                    }).then(Mono.defer(() -> reactiveCouchbaseTemplate.getCouchbaseClientFactory().getDefaultCollection().reactive().mutateIn(appIdentifier, Arrays.asList(MutateInSpec.increment("pongReceivedCount", 1),
                        MutateInSpec.upsert("pongReceived", currentTimeMillis())), MutateInOptions.mutateInOptions().expiry(Duration.ofSeconds(15)))))
                    .then(Mono.just(ResponseEntity.ok().<Void>build()))))
            .onErrorResume(CasMismatchException.class, ex -> Mono.empty())
            .onErrorResume(DocumentNotFoundException.class, ex -> Mono.empty())
